...
/**
 * Created by zhouwen on 2017/5/8.
 */
public class TaskExecutor {
    ...
    private static final ThreadFactory sThreadFactory = new ThreadFactory() {
        ...
        public Thread newThread(Runnable r) {
            LOGGER.info("TaskExecutor newThread#"+   mCount.getAndIncrement());
            return new Thread(r, "TaskExecutor newThread#" + mCount.getAndIncrement());
        }
    };
     ...
     * @param r Runnable
     */
    public static void executeMonitorTask(String threadName, Runnable r) {
        LOGGER.info("TaskExecutor>>thread name="+threadName);
        execute(r);
        sThreadPoolExecutor.addMonitorTask(threadName, new ITaskMonitor() {
            @Override
            public void beforeExecute(Thread t, Runnable r) {
                LOGGER.info("monitor>>>>beforeExecute>>>>thread_name>"+t.getName());
            }

            @Override
            public void afterExecute(Runnable r, Throwable t) {
                LOGGER.info("monitor>>>>afterExecute>>>>>"+r);
            }

            @Override
            public void terminated(int largestPoolSize, long completedTaskCount) {
                LOGGER.info("monitor>>>>terminated>>>>largestPoolSize>"+largestPoolSize+
